angular.module('quickFixModule.dropdown', [])
    .directive('cgsDropdown', function($parse, $interval) {
        ...
        return {
            ...
            link: watchModel,
            templateUrl: '/assets/js/directives/templates/dropdown.html'
        };
        ...
        function watchModel(scope, element, attrs) {
            ...
            scope.$watch("datasource.Current", function(options) {
                ...
                '<select id="select-' + scope.id + '" class="selectpicker" data-live-search="true"   data-selected-text-format="count > 4" data-width="100%"></select>' +
                '</div>');

                var icon = '<span class="glyphicon glyphicon-th"></span> &nbsp;&nbsp;'


                angular.forEach(options, function(option) {
                    var htmlContent = "<option ' data-content='" +icon+ option.value + "' value='" + option.value + "' ></option>";

                    if (option.text == null) {
                        if (option.subValue == null) {
                            htmlContent = "<option  data-content='" + icon+ option.value + "' value='" + option.value + "'></option>";
                        } else {
