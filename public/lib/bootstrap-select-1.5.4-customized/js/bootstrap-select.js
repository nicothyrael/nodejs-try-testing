  ...
  Selectpicker.prototype = {
    ...
    createLi: function () {
      ...
      this.$element.find('option').each(function (i) {
        ...
        if ($this.is(':selected')) {
          _liHtml += '<li rel="' + i + '" selected class="selected">' + _liA[_liA.length-1] + '</li>';      
        }  
        else {
          _liHtml += '<li rel="' + i + '">' + _liA[_liA.length-1] + '</li>';  
        }
      });
    ...
    liveSearchListener: function () {
      ...
      var oldValue = "";
      this.$searchbox.on('input propertychange', function (a,b) {
        if (that.$searchbox.val()) {
          if (that.$searchbox.val().indexOf(oldValue) >= 0) {
            that.$lis.not('.hide').find('a').not(':icontains(' + that.$searchbox.val() + ')').parent().addClass('hide');
          }
          else if (oldValue.indexOf(that.$searchbox.val()) >= 0) {
            that.$lis.filter('.hide').find('a').filter(':icontains(' + that.$searchbox.val() + ')').parent().removeClass('hide');
          }
          else {
            that.$lis.not('.hide').addClass('hide');
            that.$lis.find('a').filter(':icontains(' + that.$searchbox.val() + ')').parent().removeClass('hide');
          }
          oldValue = that.$searchbox.val();
